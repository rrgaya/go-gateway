...
func (g *MarketDataGateway) SubscribeMarkets(markets []gateway.Market) error {
	...
	log.Printf("ExMarkets polling for %d order book updates every %v", len(markets), refreshInterval)
	...
	for _, market := range markets {
		go func(market gateway.Market) {
			for {
				...
				if err != nil {
					log.Printf("ExMarkets market data failed to fetch order book %s, err: %s", market.Symbol, err)
...
func (g *MarketDataGateway) updateBooksFromAPI(market gateway.Market) error {
	res, err := g.api.Book(market.Symbol)
	if err != nil {
		return err
	}
...
func (g *MarketDataGateway) processAPIBookOrders(market gateway.Market, book APIBook) error {
...
func bookOrderToGtwEvent(symbol string, side gateway.Side, p APIPrice) gateway.Event {
