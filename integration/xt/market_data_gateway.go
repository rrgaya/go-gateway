...
func NewMarketDataGateway(api *API, opts gateway.Options, tickCh chan gateway.Tick) *MarketDataGateway {
...
func (g *MarketDataGateway) SubscribeMarkets(markets []gateway.Market) error {
	...
	log.Printf("XT polling for %d order book updates every %v", len(markets), refreshInterval)
	...
	for _, market := range markets {
		go func(market gateway.Market) {
			for {
				...
				if err != nil {
					log.Printf("XT market data failed to fetch order book %s, err: %s", market.Symbol, err)
