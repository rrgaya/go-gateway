...
func (g *MarketDataGateway) SubscribeMarkets(markets []gateway.Market) error {
	...
	log.Printf("P2PB2B polling for %d order book updates every %v", len(markets), refreshInterval)
	...
	for _, market := range markets {
		go func(market gateway.Market) {
			for {
				...
				if err != nil {
					log.Printf("P2PB2B market data failed to fetch order book %s, err: %s", market.Symbol, err)
...
func (g *MarketDataGateway) updateBooksFromAPI(market gateway.Market) error {
	res, err := g.api.Depth(market.Symbol)
	if err != nil {
		return err
	}
