...
func (g *MarketDataGateway) SubscribeMarkets(markets []gateway.Market) error {
	log.Printf("Hotbit opening %d websocket connections to subscribe to %d pairs", len(markets), len(markets))

	if len(g.options.Proxies) > 0 {
		log.Printf("Hotbit using %d proxy servers to distribute connections", len(g.options.Proxies))
	}
	...
	for proxy, markets := range proxyGroups {
		go func(proxy *url.URL, markets []gateway.Market) {
			for _, market := range markets {
				...
				if err != nil {
					err = fmt.Errorf("Ws proxy [%s] market [%s] failed to connect, err: %s", proxy, market, err)
					subCh <- err
					return
				}
				...
				case <-proxyAbortCh[proxy]:
					log.Printf("Ws proxy [%s] aborting subs...", proxy)
					return
				default:
					subCh <- nil
				}
	...
	for err := range subCh {
		if err != nil {
			log.Printf("Aborting rest of ws proxy connecttions...")
		...
		if count > 0 && count%50 == 0 {
			log.Printf("Hotbit ws opened %d/%d connections...", count, len(markets))
		}
	...
	log.Printf("Hotbit finished opening market data ws connections in %v....", time.Now().Sub(startAt))
...
func (g *MarketDataGateway) connectWs(market gateway.Market, proxy *url.URL, maxRetries, retryCount int) error {
	...
	if err != nil {
		log.Printf("Hotbit ws [%s] [%s] failed to connect and subscribe to market data [retry count %d], err: %s", proxy, market, retryCount, err)
	...
	go func() {
		...
		if g.options.Verbose {
			log.Printf("Hotbit market data gateway ws disconnected, err: %s", err)
	...
	go func() {
		...
		for msg := range ws.Message() {
			...
			if err != nil {
				log.Printf("Hotbit failed to process websocket message, error: %v, msg: %+v", err, msg)
				log.Printf("Hotbit ws [%s] [%s] reconnecting...", proxy, market)
				ws.Close()
				err = g.connectWs(market, proxy, maxRetries, 0)
				if err != nil {
					err = fmt.Errorf("Hotbit [%s] [%s] failed to reconnect, err: %s", proxy, market, err)
					panic(err)
				}
			}
		}
...
func (g *MarketDataGateway) subscribeDepth(ws *WsSession, market gateway.Market) error {
	subscribeParams := NewHotbitDepthSubscribeParams(market.Symbol, 100, market.PriceTick)
	subReq := WsRequest{
	...
	if REQUEST_CONFIRMATION_WAIT_MODE {
		...
		if !strings.Contains(result, "success") {
			return fmt.Errorf("Hotbit book depth %s subscribe failed, unsuccessful status, res: %s", market.Symbol, result)
		}
	} else {
		err := ws.SendMessage(&subReq)
		if err != nil {
			return fmt.Errorf("Hotbit failed send depth sub message: err: %s", err)
		}
	}
...
func (g *MarketDataGateway) subscribeDeals(ws *WsSession, market gateway.Market) error {
	...
	if REQUEST_CONFIRMATION_WAIT_MODE {
		...
		result := string(res.Result)
		if !strings.Contains(result, "success") {
			return fmt.Errorf("Hotbit deals %s subscribe failed, unsuccessful status, res: %s", market.Symbol, result)
		}
	} else {
		err := ws.SendMessage(&subReq)
		if err != nil {
			return fmt.Errorf("Hotbit failed send deals sub message: err: %s", err)
		}
	}
...
func (g *MarketDataGateway) processDeals(market gateway.Market, wsMessage WsMessage, receivedInitialSnapshot bool) error {
	...
	if len(params) < 2 {
		log.Println("Deals update params had less than 2 items", string(wsMessage.Params))
		err = errors.New("Expected trades update params array to have at least 2 items")
		return err
	}

	var deals []HotbitDeal
	var symbol string
